...
def _label_to_onehot(target, num_classes=100):
    ...
    onehot_target = torch.zeros(
        target.size(0), num_classes, device=target.device
    )
...
class GradientReconstructor:
    ...
    def __init__(
        self, model, mean_std=(0.0, 1.0), config=DEFAULT_CONFIG, num_images=1
    ):
    ...
    def reconstruct(
        ...
        if labels is None:
            ...
            if False:
                ...
                labels = (
                    last_weight_min.detach().reshape((1,)).requires_grad_(False)
                )
        ...
        return x_optimal.detach(), (
            stats | {"grad_sim_per_module": self.grad_sim_per_module}
        )
        ...
        if self.reconstruct_label:
            ...
            if self.config["optim"] == "adam":
                optimizer = torch.optim.Adam(
                    [x_trial, labels], lr=self.config["lr"]
                )
            elif self.config["optim"] == "sgd":  # actually gd
                optimizer = torch.optim.SGD(
                    [x_trial, labels], lr=0.01, momentum=0.9, nesterov=True
                )
        ...
        try:
            for iteration in range(max_iterations):
                closure = self._gradient_closure(
                    optimizer, x_trial, input_data, labels
                )
                ...
                with torch.no_grad():
                    ...
                    if (
                        ...
                        print(
                            f"It: {iteration}. Rec. loss: {rec_loss.item():2.4f}.",
                            flush=True,
                        )
    ...
    def _gradient_closure(self, optimizer, x_trial, input_gradient, label):
        def closure():
            ...
            self.log_grad_sim(input_gradient, gradient)
    ...
    grad_sim_per_module = defaultdict(list)

    def log_grad_sim(self, input_grad, grad):
        for i, (name, _) in enumerate(list(self.model.named_parameters())):
            self.grad_sim_per_module[name].append(
                torch.cosine_similarity(
                    input_grad[i].view(1, -1), grad[i].view(1, -1)
                ).item()
            )
    ...
    def _average_trials(self, x, labels, input_data, stats):
        print(
            f'Computing a combined result via {self.config["scoring_choice"]} ...'
        )
...
def reconstruction_costs(
    ...
    for trial_gradient in gradients:
        ...
        if indices == "topk-2":
            _, indices = torch.topk(
                torch.stack([p.norm().detach() for p in trial_gradient], dim=0),
                4,
            )
        for i in indices:
            if cost_fn == "l2":
                ...
                costs -= (
                    trial_gradient[i] * input_gradient[i]
                ).sum() * weights[i]
