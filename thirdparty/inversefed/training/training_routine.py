...
def validate(model, loss_fn, dataloader, defs, setup, stats):
    ...
    with torch.no_grad():
        for batch, (inputs, targets) in enumerate(dataloader):
            ...
            inputs = inputs.to(**setup)
            targets = targets.to(
                device=setup["device"], non_blocking=NON_BLOCKING
            )
...
def prune(gradient, percent):
    ...
    # Workaround for https://github.com/pytorch/pytorch/issues/68525
...
def step_with_defense(
    model, loss_fn, dataloader, optimizer, scheduler, defs, setup, stats, opt
):
    """Step through one epoch."""
