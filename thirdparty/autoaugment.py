...
def img_zoom(img):
    ...
    img1 = transforms.RandomCrop(img.shape[1] + pad_len, padding=pad_len)(img)
    img2 = transforms.Resize(img.shape[1])(img1)
    img2 = np.array(img2)
    mask = (img2 == 0) * 1
    img2 = (1 - mask) * img2 + mask * (
        np.random.random(img2.shape) * 255
    ).astype(np.uint8)
...
class SubPolicy(object):
    def __init__(self, p1, operation1, magnitude_idx1, fillcolor=(0, 0, 0)):
        self.args = {}
        self.args["p1"] = p1
        self.args["operation1"] = operation1
        self.args["magnitude_idx1"] = magnitude_idx1
        ...
        func = {
            ...
            "zoom": lambda img, magnitude: img_zoom(img),  # New
            "warp": lambda img, magnitude: img_warp(img),  # New
        }
    ...
    def __call__(self, img):
        try:
            ...
            # Hacky solution, runs only once in the beginning.
            # It changes fillcolor to int in case we're dealing with grayscale images
            fillcolor = 0
            ranges = {
                "shearX": np.linspace(0, 0.3, 10),
                "shearY": np.linspace(0, 0.3, 10),
            ...
            func = {
                ...
                "rotate": lambda img, magnitude: rotate_with_fill(
                    img, magnitude
                ),
                "color": lambda img, magnitude: ImageEnhance.Color(img).enhance(
                    1 + magnitude * random.choice([-1, 1])
                ...
                "autocontrast": lambda img, magnitude: ImageOps.autocontrast(
                    img
                ),
                "equalize": lambda img, magnitude: ImageOps.equalize(img),
                "invert": lambda img, magnitude: ImageOps.invert(img),
            ...
            self.operation1 = func[self.args["operation1"]]
            self.magnitude1 = ranges[self.args["operation1"]][
                self.args["magnitude_idx1"]
            ]
            img = self.operation1(img, self.magnitude1)
        return img
